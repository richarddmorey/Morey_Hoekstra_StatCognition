...
loglike(g$par, freqs, return_probs = TRUE) %>%
  `[`(,c(2,1,3)) %>%
  data.frame() %>%
...
error_rates %>%
  mutate(response = tools::toTitleCase(response)) %>%
  ggplot(aes(x=effect_size,y=predicted,group=response,color=response)) +
  ...
  scale_color_manual(name="Response", values = c("#FF0000", "#999999", "#0000FF")) +
  scale_fill_manual(name="Response",values = c("#FF0000", "#999999", "#0000FF")) +
