...
Participants were scientists or trainees recruited via social media. Our statistical reasoning task required them to perform a series of experiments to judge which of two groups of Christmas elves --- "Jinglies" or "Sparklies" --- could make more of a particular toy, where the true effect size was a randomly-assigned group difference between 0 and 1 standard deviations ($\delta$). The numerical information for an experiment, including sample sizes and the test statistic, was translated into color and location and displayed as a point on a two-dimensional visual interface (Figure \ref{fig:interface}). Participants could change the sample size per group for each experiment (increasing the time required to return a result), but did not know its numerical value. The interpretation of these colors and locations was unknown to the participants, aside from the monotone relationship with the statistical evidence.
...
Of the `r null_n` participants for whom the null hypothesis was true (i.e. $\delta=0$), `r t1_err_count` participants (`r round(t1_err_count/null_n * 100, 0)`%) incorrectly indicated an effect. This is larger than the typically-accepted 5% false positive rate in many sciences; however, participants were performing a novel task with no recourse to numbers or statistical software. Those who did not indicate an effect when $\delta=0$ tended to indicated that they *did not detect* an effect (`r null_nodet_count`; `r round(null_nodet_count/null_n * 100,0)`%), which is the correct conclusion from the SST perspective. The other `r null_same_count` (`r round(null_same_count/null_n * 100,0)`%) indicated that the groups were the same, which under SST is a fallacy.
...
We determined the coding scheme and independently categorized the first 20 participant, discussing the source of disagreements. After categorizing the remaining participants, some disagreements were resolved through agreement, and a discussion between the authors was had over what caused the disagreements. The remainder of the disagreements were re-coded separately, and a final round of discussion resolved the remaining disagreements. Participants' codings described in detail in Supplement B.
