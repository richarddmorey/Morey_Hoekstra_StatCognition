...
Instead of the usual human-factors approach, which adopts familiar statistical language and tests participants' fluency with existing procedures, we adopt a more cognitive perspective. We aim to test working scientists' understanding of the basic conceptual framework underlying SST using a simulated experimental task. To avoid conflating basic reasoning processes and fluency with common statistical terminology, we avoid using common statistics---or, indeed, any numbers---at all.
...
Participants sought out information that would be necessary for significance tests. They made heavy use of shuffle reports (Figure~{=latex}\@ref(fig:sampling)A). Across all true effect sizes, participants sampled a median of 152 shuffle reports (range: 1-2034; in both panels A and B, lines show robust regression fits [@Venables:Ripley:2002]).
